...
2. 进入插件配置页，右侧会弹出一个 JSON 代码编辑器（字段 `rules_json`）。默认已经放入两条示例规则，演示「先替换为 123、再把 123 变成 1」的流水线。
   - 每条规则都是一个对象：`{name, scope, pattern, replacement, flags, order, enabled}`。
   - 直接修改示例对象即可自定义；需要更多规则时在数组中新增对象（注意用逗号分隔）。
   - `order` 数值越小越先执行，也可以通过上下拖动语句块来调整顺序。
3. 保存配置后，新的正则流水线立即生效。

> 示例顺序效果：若规则 1 将所有文本替换为 `123`，规则 2 将 `123` 替换成 `1`，则按「规则 1 → 规则 2」执行的最终结果是 `1`；若顺序取反则输出为 `123`。此示例正是配置默认值中提供的样例，方便你直接测试。
...
| `rules_json` | `text` | 预置 2 条示例规则 | 以 JSON 数组维护正则流水线，支持代码编辑器模式。 |
| `rules` | `list<object>` | `[]` | 旧版字段，为兼容保留，可忽略。 |

### JSON 规则编辑提示

- **保持数组结构**：整个字段必须是 `[...]` 形式。多条规则之间请用逗号分隔。
- **常见字段**：
  - `name`：规则名称，建议写清处理意图。
  - `scope`：取值 `user_input`、`ai_output` 或 `both`。
  - `pattern` / `replacement`：Python `re` 语法，`replacement` 可使用捕获组（如 `\1`）。
  - `flags`：写成字符串数组，例如 `["IGNORECASE","DOTALL"]`。
  - `order`：数字越小越靠前，默认样例为 10、20。
  - `enabled`：是否启用该规则。
- **快速复用默认示例**：默认两条规则演示“先归一化为 123，再裁剪到 1”的效果。可以先在示例上修改正则，确认逻辑无误后再扩展更多条目。
- **错误排查**：JSON 结构错误或字段缺失时，插件会在日志中输出警告；留空 `pattern` 会自动跳过该条规则。
